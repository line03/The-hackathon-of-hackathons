...
function GenderPage() {
    ...
    const [userAnswers, setUserAnswers] = useState<GenderUserAnswer[]>([]);
    ...
    const loadQuizData = async () => {
        setIsLoading(true);
        try {
            const data = quizData as any;
            const questions = data.modules 
                ? data.modules.flatMap((module: any) => module.quiz)
                : (data.quiz || []);
        ...
        const runAnalysis = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    navigate(ROUTES.THEATRE_WORLD_GENDER_CORRECTION, {
                        state: {
                            analysisResult: data,
                            score,
    ...
    const handleSubmit = () => {
        ...
        // If it's the first question (index 0), show the second video
    ...
    useEffect(() => {
        if (showSecondVideo && videoRef.current) {
            videoRef.current.play();
        }
    }, [showSecondVideo]);
    ...
    if (!quizStarted) {
        if (!mainQuizCompleted) {
            return (
                <WorldPageLayout backgroundImage="/gender.png" altText="Gender Quiz" onBackClick={handleBackClick}>
                    <div className="quiz-start-container">
                        <h2 className="quiz-start-title">Complete Main Quiz First</h2>
                        <p className="quiz-start-description">Please complete the main Image Theatre quiz before accessing this module.</p>
                        <button className="start-quiz-button" onClick={() => navigate(ROUTES.THEATRE_WORLD_IMAGE_THEATRE)}>Go to Main Quiz</button>
                    </div>
                </WorldPageLayout>
            );
        }
    ...
    return (
        <WorldPageLayout backgroundImage="/gender.png" altText="Gender Quiz" onBackClick={handleBackClick}>
            <div className="it-character-container" style={{ display: (showIntroVideo || showSecondVideo) ? 'none' : 'block' }}>
            ...
            <div className="cr-quiz-container" style={{ display: (showIntroVideo || showSecondVideo) ? 'none' : 'block' }}>
                ...
                <div className="answers-section">
                    {currentQuestion.answerOptions.map((option, index) => {
                        ...
                        if (selectedAnswer === index && !showRationale) buttonClass += ' selected';
                        else if (showRationale) {
                            if (selectedAnswer === index) buttonClass += option.isCorrect ? ' correct' : ' incorrect';
                ...
                <div style={{ display: 'flex', flexDirection: 'row', gap: '1rem', marginBottom: '1.2rem', alignItems: 'center', justifyContent: 'space-between' }}>
                    {!showHint && !isSubmitted && <button className="hint-button" onClick={handleShowHint}>💡 Show Hint</button>}
                    {showHint && <div></div>}
                    {selectedAnswer !== null && !isSubmitted && <button className="submit-button" onClick={handleSubmit}>Submit Answer</button>}
                ...
                {isSubmitted && !showSecondVideo && (
                    <div className="post-submit-actions">
                        ...
                        <button className="submit-button" onClick={handleNext}>{isLastQuestion ? 'Finish Quiz' : 'Next Question →'}</button>
                    </div>
                )}
            </div>
            ...
            {showIntroVideo && (
                <div className="video-overlay">
                    <video ref={videoRef} src="/TheatreVideo/gender.mp4" onEnded={handleVideoEnd} className="quiz-video" />
                </div>
            )}

            {showSecondVideo && (
                <div className="video-overlay">
                    <video ref={videoRef} src="/TheatreVideo/gender2.mp4" onEnded={handleSecondVideoEnd} className="quiz-video" />
                </div>
            )}
        </WorldPageLayout>
    );
