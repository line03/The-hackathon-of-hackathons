...
type Mode = 'start' | 'quiz';

type CharacterState = 'idle' | 'happy' | 'sad';

// Map character state to Theatre character video files in public/Theatre
...
interface ImageTheatreUserAnswer {
    question: string;
    answerOptions: Question['answerOptions'];
    answerCorrect: boolean;
}

function ImageTheatrePage() {
    ...
    // AI analysis state
    const [userAnswers, setUserAnswers] = useState<ImageTheatreUserAnswer[]>([]);
    ...
    const loadQuizData = () => {
        try {
            // Image Theatre quiz.json structure: { "quiz": [...] }
            const data = quizData as { quiz?: Question[]; modules?: any[] };
            
            // Handle both structures: if it has modules, use that; otherwise use quiz directly
    ...
    useEffect(() => {
        // Prime quiz metadata logs, but do not auto-start quiz
        if (quizData) {
            const data = quizData as { quiz?: Question[]; modules?: any[] };
            const questionCount =
                data.quiz?.length || (data.modules?.flatMap((m: any) => m.quiz || []).length || 0);
            if (questionCount > 0) {
                console.log(`Image Theatre quiz loaded: ${questionCount} questions available`);
            }
        }
    }, []);
    ...
    const handleNext = () => {
        if (isLastQuestion) {
            // Mark main Image Theatre quiz as completed
            markCompleted('image-theatre-main');
            // Trigger AI analysis and navigate to correction page
    ...
    // Load user preferences for AI analysis (if set elsewhere)
    useEffect(() => {
        ...
        const runAnalysis = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    navigate('/map/theatre-world/image-theatre/correction', {
                        state: {
                            analysisResult: data,
                            score: userAnswers.filter((a) => a.answerCorrect).length,
            ...
            // Fallback: navigate with simple, static explanations if AI is unavailable
    ...
    if (mode === 'start') {
        return (
            <WorldPageLayout
                ...
                altText="Image Theatre Quiz"
                onBackClick={handleBackClick}
            >
                <div className="quiz-start-container">
                    <h2 className="quiz-start-title">Ready to Test Your Knowledge?</h2>
                    <p className="quiz-start-description">
                        Answer questions about Image Theatre to test your understanding.
                    </p>
                    <button
    ...
    if (mode === 'quiz' && allQuestions.length === 0 && !isAnalyzing) {
        return (
            <WorldPageLayout
                backgroundImage="/Theatre/Image_Theatre/Image_Theatre.png"
                altText="Image Theatre Quiz"
    ...
    return (
        <WorldPageLayout
            ...
            onBackClick={handleBackClick}
        >
            {/* Theatre character container - mirrors Criminalization layout */}
                    ...
                    <>
                        ...
                        <div className={`rationale-popup ${isCorrect ? 'correct' : 'incorrect'}`}>
                            <div className="rationale-header">
                                ...
                                <button
                                    className="close-rationale-button"
...
export default ImageTheatrePage;
